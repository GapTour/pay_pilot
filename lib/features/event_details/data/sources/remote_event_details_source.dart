...
class RemoteEventDetailsSource implements IEventDetailsSource {
  final EventDetailsApiProvider _apiProvider;
  RemoteEventDetailsSource(this._apiProvider);
  ...
  Future<DataState<ResponseOrder>> changeOrderStatus(
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        final order = ResponseOrder.fromApi(rawData);
  ...
  Future<DataState<int>> deleteTransaction(TransactionParams params) async {
    try {
      final Response response = await _apiProvider.deleteTransaction(
        params.id!,
      );
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final orders = rawData.map((e) {
          return ResponseOrder.fromApi(e);
        }).toList();
  ...
  Future<DataState<ResponseEventDetails>> getEvent(int id) async {
    try {
      ...
      if (response.statusCode == 200) {
        final rawData = response.data['data'];
        final eventDetails = ResponseEventDetails.fromApi(rawData);
  ...
  Future<DataState<ResponseEventRatio>> insertRatio(
    ...
    try {
      ...
      if (response.statusCode == 201) {
        final rawData = response.data['data'];
        final eventRatio = ResponseEventRatio.fromApi(rawData);
